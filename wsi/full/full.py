...
parser.add_argument('--cfg_path', default="/home/omnisky/ajmq/patch_slide_local/configs/resnet34_full.json",
                    metavar='CFG_PATH',
                    type=str,
                    help='Path to the config file in json format')
parser.add_argument('--save_path', default="/home/omnisky/ajmq/patch_slide_relate/save/full6.10", metavar='SAVE_PATH',
                    type=str,
...
def run(args):
    ...
    model_crf, model_mil = produce_model_full(cfg)
    ...
    for epoch in range(cfg['epoch']):
        summary_train = train_gan(dataloader_train_mil, model_crf, model_mil, base_dataset, summary_train, num_workers,
                                  batch_size_train, loss_fn_without_sigmoid, optimizer_crf, optimizer_mil, cfg['top_k'],
                                  cfg['pre_value'], summary_writer, cfg, record_list_total_train, record_list_key_train)
