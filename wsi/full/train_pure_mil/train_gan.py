...
def train_gan(dataloader, model_crf, model_mil, dataset, summary, num_workers, batch_size, loss_fn, optimizer_crf,
    ...
    for step, (data_crf, target_crf, data_mil, target_mil, patch, position) in enumerate(train_loader):
        ...
        show_mil(loss_crf_mil, predict_mil, step, summary, target_mil, time_now, summary_writer, cfg,
                 record_list)
...
def show_mil(loss_crf_mil, predict_mil, step, summary, target_mil, time_now, summary_writer, cfg,
                 record_list):
    ...
    print("loss_mil: ", str(loss_crf_mil))
    ...
    print_section("", [summary['epoch'] + 1, step + 1, loss_crf_mil, acc_mil, time_spent],
                  mode="sample")
    ...
    if summary['step'] % cfg['log_every'] == 0:
        summary_writer.add_scalar('train_step/loss_mil', float(loss_crf_mil.cpu()), summary['step'])
