...
def train_epoch(summary, summary_writer, cfg, model, loss_fn, optimizer,
    ...
    for step in range(steps):
        ...
        idx_rand = Variable(
            torch.randperm(batch_size * 2).cuda(non_blocking=True))
...
def run(args):
    ...
    if cfg['other_dict']==1:
        ...
        stat_dict = {k: v for k, v in stat_dict.items() if k in model_dict.keys() and k!='module.fc.weight' and k!='module.fc.bias'}
