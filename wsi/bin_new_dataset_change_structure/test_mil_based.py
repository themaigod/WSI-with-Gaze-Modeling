...
# !!!!!!!!!!!!
# batch_size禁止为1
# （对1没有进行特殊处理，dataloader输出少一维， 可能会产生各种问题）
# 同时建议drop_last设置为True以防止batch_size为1的情况出现
...
def run(args):
    ...
    summary = {'epoch': 0, 'loss': float(0), 'loss_mil': float(0), 'acc': 0, 'acc_crf': 0, 'fp': 0,
               "fn": 0, "tp": 0, "tn": 0}
    ...
    summary_writer.add_scalar('test_epoch/loss', summary['loss'] / len(test_loader), summary['epoch'])
    summary_writer.add_scalar('test_epoch/acc', summary['acc'] / len(test_loader), summary['epoch'])
    summary_writer.add_scalar('test_epoch/tpr', tpr, summary['epoch'])
    summary_writer.add_scalar('test_epoch/fpr', fpr, summary['epoch'])
    ...
    torch.save({"summary": summary, "len": len(test_loader), "loss_final": summary['loss'] / len(test_loader),
                ...
                "acc_crf": summary['acc_crf'] / len(test_loader), "tpr": tpr,
                "fpr": fpr}, os.path.join(args.save_path, 'test_result.ckpt'))

    result_mil, result_label = base_dataset.calc_result(2)
...
def load_weight(args, model_crf, model_mil):
    ...
    model_crf.load_state_dict(weight['state_dict_crf'])
    model_mil.load_state_dict(weight['state_dict_mil'])
